...
SELECT pass('Test transfer_organization_ownership!');
...
select results_eq('select username, is_owner from app_public.organization_memberships om join app_public.users u on om.user_id=u.id join app_public.organizations o on (o.id=om.organization_id) where o.slug=' || quote_literal('dolls') ||'::citext  order by username',
       $$VALUES('gd'::citext, true), ('jmarca'::citext, false) $$,
       'Should be able to view that gd is owner, jmarca is not owner');

-- cannot transfer ownership of dolls group to jmarca (not current owner)
select results_eq(
'with
...
|| quote_literal('jmarca')
|| ')
select app_public.transfer_organization_ownership(o.id,u.id)
from o join u on (true)',
$$VALUES ( NULL::organizations )$$,
'Should return empty set when trying to change ownership when not owner');
...
select results_eq('select username, is_owner from app_public.organization_memberships om join app_public.users u on om.user_id=u.id join app_public.organizations o on (o.id=om.organization_id) where o.slug=' || quote_literal('marca') ||'::citext  order by username',
       $$VALUES('gd'::citext, false), ('jmarca'::citext, true) $$,
       'Should be able to view that gd is not owner, jmarca is owner');


-- should not be able to transfer ownership to non-member
select results_eq(
'with
o(id) as (select id from app_public.organizations o where slug='
|| quote_literal('marca')
...
from o join u on (true)',
$$VALUES ( NULL::organizations )$$,
'Should return empty set when trying to change ownership to non-member');
...
|| ')
select ao.*
from  o, u, app_public.transfer_organization_ownership(o.id,u.id) ao',
'organization_is',
'Should return the organization when ownership is successfully changed');

select results_eq('select username, is_owner from app_public.organization_memberships om join app_public.users u on om.user_id=u.id join app_public.organizations o on (o.id=om.organization_id) where o.slug=' || quote_literal('marca') ||'::citext  order by username',
       $$VALUES('gd'::citext, true), ('jmarca'::citext, false) $$,
       'Should have switched ownership from jmarca to gd');
