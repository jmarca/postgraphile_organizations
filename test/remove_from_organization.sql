...
SELECT pass('Test remove_from_organization!');
...
-- cannot remove gd from dolls group

with
o(id) as (select id from app_public.organizations o where slug='dolls'),
u(id) as (select id from app_public.users u where username='gd')
select app_public.remove_from_organization(o.id,u.id)
...
select results_eq('select username from app_public.organization_memberships om join app_public.users u on om.user_id=u.id join app_public.organizations o on (o.id=om.organization_id) where o.slug=' || quote_literal('dolls') ||'::citext  order by username',
       $$VALUES(('gd'::citext)), (('jmarca'::citext)) $$,
       'Should be able to view the current members of an organization');

-- can remove gd from marca group
...
select results_eq('select username from app_public.organization_memberships om join app_public.users u on om.user_id=u.id join app_public.organizations o on (o.id=om.organization_id) where o.slug=' || quote_literal('marca') ||'::citext  order by username',
       $$VALUES(('jmarca'::citext)) $$,
       'Should have removed gd from marca organization');

-- cannot remove self from marca group, because owner of it

with
o(id) as (select id from app_public.organizations o where slug='marca'),
u(id) as (select id from app_public.users u where username='jmarca')
select app_public.remove_from_organization(o.id,u.id)
from o join u on (true);
